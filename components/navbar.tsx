      ...
      <div className='flex justify-between mx-auto z-99 fixed top-0 left-0 right-0 flex-col backdrop-blur-2xl'>
        <div className='flex md:w-8/12 md:mx-auto justify-between p-5'>
          ...
          <ol className='flex gap-8 font-light items-center justify-center text-sm [&>li]:hover:translate-y-0.5 [&>li]:cursor-pointer '>
            <li className='md:block hidden transition-all duration-300'>Write</li>
            <li className='transition-all duration-300'><button className='px-4 py-2 rounded-full bg-black text-white cursor-pointer' onClick={session?.user?.name?()=>redirect("/home/profile"):() => { setHide(false) }}>{session?.user?.name?<div className='flex items-center gap-2'><img src={session.user.image || "./default-avatar.png"} alt="" className='h-6 rounded-full' />{session.user.name}</div>:"Get Started"}</button></li>
          </ol>
        </div>
        <hr />
      </div>
