        ...
        <AnimatePresence>
            {Hide ? "" :
                <motion.div onClick={() => setHide(true)} className={`z-999 flex fixed top-0 left-0 right-0 bottom-0 justify-center items-center backdrop-blur-sm backdrop-brightness-50 text-black`}>
                    <motion.div onClick={(e) => { e.stopPropagation() }} initial={{ scale: 0.95 }} animate={{ scale: 1 }} transition={{ duration: 0.15, type: 'keyframes' }} exit={{ scale: 0.95 }} className='mx-auto bg-white flex flex-col rounded-md text-center items-center p-16 min-h-2/4 justify-around gap-2'>
                        <p className='primary-font text-3xl'>Join Medium Clone.</p>
                        ...
                        <hr className='border-1/2 w-full' />
                        <form onSubmit={handleSubmit} className='flex flex-col items-center gap-2 text-center justify-center w-90    p-2'>
